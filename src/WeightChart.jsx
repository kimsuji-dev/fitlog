...
export default function WeightChart({ weights, periods }) {
  ...
  // y축 눈금: min/max 두 개만 (귀엽고 단순하게)
  const yTicks = [yMin + kgPad, yMax - kgPad]

  return (
    <div className="card">
      <div>⚖️ 몸무게 추이</div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ overflow: 'visible' }} role="img" aria-label="몸무게 변화 그래프">
        {bands.map((b, i) => (
          <rect key={i} x={b.x1} y={PAD.top} width={Math.max(1, b.x2 - b.x1)} height={innerH} fill={PERIOD_FILL} />
        ))}
        {yTicks.map((t, i) => (
          <g key={i}>
            <line x1={PAD.left} y1={yFor(t)} x2={W - PAD.right} y2={yFor(t)} stroke="var(--border-color)" strokeWidth="1" />
        ...
        {points.map((p, i) => (
          <rect key={i} x={p.x - POINT_SIZE / 2} y={p.y - POINT_SIZE / 2} width={POINT_SIZE} height={POINT_SIZE} fill={POINT_COLOR} stroke="#fdf8f4" strokeWidth="1" />
        ))}
